document.addEventListener("DOMContentLoaded", function () {
  ...
  function setSelection(row) {
    ...
    // hiddenに反映
    if (editLocationIdInput) editLocationIdInput.value = selectedId;
    if (deleteLocationIdInput) deleteLocationIdInput.value = selectedId;
  ...
  if (editBtn && editForm) {
    editBtn.addEventListener("click", (e) => {
      if (!editLocationIdInput?.value) {
        e.preventDefault();
        alert("編集する管理場所を選択してください。");
      }
    });
  }
  ...
  if (deleteBtn && deleteForm) {
    deleteBtn.addEventListener("click", (e) => {
      ...
      const selectedId = deleteLocationIdInput?.value;
